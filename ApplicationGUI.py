...
class Application():
    ...
    def init_widgets(self):
        ...
        self.seq_tab = SequenceReplaceTab.SequenceReplaceTab(
            n_book,
            self.general,
            name="Sequence Replace"
        )
        self.sum_tab = SummaryTab.SummaryTab(
            n_book, self.general, name="LIN Summary")
        n_book.pack(expand=1, fill="both")
        self.renum_tab = RenumberTab.RenumberTab(
            n_book, self.general, name="LINE Renumber")
        n_book.pack(expand=1, fill="both")
    ...
    def load_defaults(self):
        try:
            with open(self.default_file, "r") as file:
                ...
                try:
                    self.defaults = {x.split("=")[0]: x.split(
                        "=")[1].strip('"') for x in data}
                except Exception:
                    print(data)
                self.keys = {k: self.defaults.get(k, "")
                             for k in self.defaults
                             if k in ["{lookup_folder}",
                                      "{inter_folder}",
                                      "{output_folder}"]}

                for cat in self.defaults:
                    for key in self.keys:
                        self.defaults[cat] = self.defaults[cat].replace(
                            key, self.keys[key])

                self.general.files["ops"].set(self.defaults["operator_file"])
                self.general.files["b_nod"].set(
                    self.defaults["bus_node_lookup"])
                self.general.files["r_nod"].set(
                    self.defaults["rail_node_lookup"])
                self.general.files["mode"].set(self.defaults["mode_lookup"])
                self.general.files["user_p"].set(
                    self.defaults["patching_overrides"])
                ...
                self.general.inter_folder = self.defaults["inter_dir_name"]
                self.general.head_name.set(self.defaults["headway_names"])
                self.general.head_defs.set(
                    self.defaults["headway_definitions"])
                for i in range(len(self.general.date_from)):
                    self.general.date_from[i].set(
                        self.defaults["date"].split("/")[i])
                for i in range(len(self.defaults["selected_day"])):
                    self.general.selected_days[i].set(
                        int(self.defaults["selected_day"][i]))

                self.ra_tab.files["stock"].set(
                    self.defaults["rolling_stock"]
                )
                self.ra_tab.files["t_name"].set(
                    self.defaults["r_stock_tiplocs"]
                )

                self.ra_tab.node_lookup.set(self.defaults["rail_node"])
                self.ra_tab.link_lookup.set(self.defaults["rail_link"])
                self.bu_tab.node_lookup.set(self.defaults["bus_node"])
                self.bu_tab.link_lookup.set(self.defaults["bus_link"])

                self.ra_tab.log.add_message(
                    "Loaded Default Variables", color="GREEN")
                self.bu_tab.log.add_message(
                    "Loaded Default Variables", color="GREEN")
    ...
    def save_defaults(self):
        try:
            ...
            self.defaults["inter_dir_name"] = self.general.inter_folder
            self.defaults["headway_names"] = self.general.head_name.get()
            self.defaults["headway_definitions"] = self.general.head_defs.get()
            self.defaults["date"] = "/".join([x.get()
                                              for x in self.general.date_from])
            self.defaults["selected_day"] = "".join(
                [str(x.get()) for x in self.general.selected_days])

            self.defaults["rolling_stock"] = self.ra_tab.files["stock"].get()
            self.defaults["r_stock_tiplocs"] = self.ra_tab.files["t_name"].get()
            ...
            self.ra_tab.log.add_message(
                "Saved Default Variables", color="GREEN")
            self.bu_tab.log.add_message(
                "Saved Default Variables", color="GREEN")

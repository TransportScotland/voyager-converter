...
class LabelledEntry:
    ...
    def add_directory(self, working_dir, change_callback=None):
        ...
        button = ttk.Button(self.frame, text="Browse",
                            command=self.get_directory_name)
        button.pack(side="left", anchor="e", padx=self.px)
...
class SelectBox:
    ...
    def __init__(self, frame, h, w, r, c, elements=[], span=4,
                 select_mode=tk.MULTIPLE):
        self.contents = []
        self.list = tk.Listbox(frame, height=h, width=w,
                               selectmode=select_mode)
        self.list.grid(row=r, column=c, rowspan=span)
        self.scroll = tk.Scrollbar(frame)
        self.scroll.grid(row=r, column=c, sticky=(
            tk.E, tk.N, tk.S), rowspan=span)

...
from WidgetTemplates import CreateToolTip, LabelledEntry, TextLog
from WidgetTemplates import ThreadWithReturn, SelectBox
...
class BusTab:
    def __init__(self, note_book, general, name="Bus", default_vars={}):
        ...
        self.header_style.configure("Head.TLabel", font=(
            "Helvetica", 10), foreground="blue")
    ...
    def read_xml_files(self, summary=False):
        ...
        if self.is_ncsd_data.get() == 1:
            threading.Thread(
                target=bi_ncsd.import_XML_data_callback, args=f_args).start()
        else:
            threading.Thread(
                target=bi.import_XML_data_callback, args=f_args).start()
    ...
    def create_patching_window(self):
        ...
        def callback_patch_routes():
            try:
                ...
                args = (node_file.get(), node_file.get(),
                ...
                t = ThreadWithReturn(
                    target=patch.patch_selected, args=args, kwargs=kwargs)
                t.start()
        ...
        def callback_make_overrides():
            patch.create_override_file(
                path_out, self.gen.files["user_p"].get())
        ...
        def callback_view_file(file_num):
            ...
            try:
                ...
                patch_log.add_message(
                    "File does not exist: Patch the routes first", color="RED")
        ...
        file_names = {1: path_out, 2: err_out,
                      3: self.gen.files["user_p"].get(),
        ...
        options_frame.pack()
        label = ttk.Label(options_frame, text="Check Size")
        label.pack(side="left")
        CreateToolTip(
            label,
            text=("Number of nodes to check when 'Use Distance' "
        ...
        distance = ttk.Checkbutton(
            options_frame, text="Use Distance", variable=use_distance)
        distance.pack(side="left")
        CreateToolTip(
            distance,
            text=("If ticked, the distance of the links will"
        ...
        patch_button = ttk.Button(button_frame, text="Patch Routes",
                                  ...
                                  style="DS.TButton")
        patch_button.pack(side="left", expand=True,
                          fill=tk.X, ipady=5, ipadx=5)
        ...
        CreateToolTip(
            view_paths, text="View the patches that the process created")
        ...
        CreateToolTip(
            view_errs, text="View the routes that could not be patched")
        view_overrides = ttk.Button(button_frame, text="Overrides",
                                    command=lambda: callback_view_file(3))
        view_overrides.pack(side="left", expand=True,
                            fill=tk.X, ipady=5, ipadx=5)
        CreateToolTip(
            view_overrides,
            text="Open the user overrides file to manually enter patches"
        )
        ...
        CreateToolTip(
            view_patch, text="View the file with all patched services")
        patch_button = ttk.Button(
            button_frame,
            text="Make Overrides",
            command=callback_make_overrides,
            style="DS.TButton"
        )
        patch_button.pack(side="left", expand=True,
                          fill=tk.X, ipady=5, ipadx=5)
        CreateToolTip(patch_button,
                      text=("Override the selected Patch Override "
    ...
    def read_file(self, key):
        if key == "XML":
            ...
            try:
                ...
                if len(ops) == 0:
                    self.log.add_message(
                        "No operators in file\nTry importing some data first",
                        color="RED"
                    )
            except IOError:
                self.log.add_message(
                    "No operator file present\nTry importing some data first",
                    color="RED"
                )
            self.update_input(ops)
        elif key == "fil":
            custom_headways = (self.gen.head_defs.get(),
                               self.gen.head_name.get())
            ...
            bi.print_lin_file(self.to_path("XML_p"), self.to_path("lin"),
                              self.gen.to_path("ops"), custom_headways,
                              self.log)
            if self.gen.auto_open.get() == 1:
    ...
    def create_header_widget(self, frame):
        label = ttk.Label(
            frame,
            text=("Import bus timetable data from "
    ...
    def create_print_lin_widgets(self, frame):
        ...
        print_button = ttk.Button(frame, text="4. Print LIN File",
                                  style="DS.TButton",
    ...
    def create_filter_widgets(self, frame):
        ...
        valid_entry = ttk.Entry(
            frame, width=50, textvariable=self.files["XML_f"])
    ...
    def create_operator_widgets(self, frame):
        ...
        op_label = ttk.Label(frame,
                             text=("-> Read in the operator file then "
        ...
        self.op_read = ttk.Button(
            frame, text="Read Operator File",
            command=lambda: self.read_file("ops")
        )
        self.op_read.grid(
    ...
    def create_headway_widgets(self, frame):
        description = ttk.Label(
            frame,
            text=("Specify Headway Periods Using a "
    ...
    def create_file_widgets(self, frame):
        ...
        butt_frame.pack()
        ncsd_check = ttk.Checkbutton(
            butt_frame, text="Is NCSD", variable=self.is_ncsd_data)
        ncsd_check.pack(side="top", padx=5, pady=5, ipadx=5, ipady=5)
        CreateToolTip(ncsd_check,
                      text=("Tick if the data being imported is NCSD\n"
                            "This data has a different format so "
                            "must be read in separately (Experimental)"))
        a = LabelledEntry(butt_frame, "Summary Output Path",
                          self.files["XML_SUMMARY"],
    ...
    def add_frames(self):
        ...
        self.file_select_frame = add_frame(
            self.master, "Select Input and Output files", [2, 1],
            c_span=2, s="we"
        )
        self.operator_frame = add_frame(
            self.master, "Filter Operators", [3, 1], c_span=2, s="we")
        ...
        self.progress_frame = add_frame(
            self.master, "Progress", [8, 1], c_span=2)

    def set_defaults(self):
        f = self.files
        f["XML"].set("..\\Data\\Bus\\Bus Data Test")
        f["XML_a"].set(os.path.relpath(os.path.join(
            self.gen.inter_dir, "headways.csv"), self.current_dir))
        f["XML_f"].set(os.path.relpath(os.path.join(
            self.gen.inter_dir, "headways_filtered_operators.csv"),
            self.current_dir))
        f["XML_p"].set(os.path.relpath(os.path.join(
            self.gen.inter_dir, "headways_patched.csv"), self.current_dir))

...
class Multicolumn_Listbox(object):
    ...
    def __init__(self, master, columns, data=None, command=None, sort=True, select_mode=None, heading_anchor = CENTER, cell_anchor=W, style=None, height=2, padding=None, adjust_heading_to_content=True, stripped_rows=None, selection_background=None, selection_foreground=None, field_background=None, heading_font= None, heading_background=None, heading_foreground=None, cell_pady=2, cell_background=None, cell_foreground=None, cell_font=None, headers=True, data_type="text", non_edit_column=[], round=True):
        ...
        self.non_edit_column = non_edit_column
        ...
        for i in range(0, self._number_of_columns):
            ...
            if adjust_heading_to_content:
                ...
                self.interior.column(i, width=10)

            self.interior.column(i, anchor=cell_anchor)
            
            self.configure_column(i,width=Font().measure(columns[i]) + 20)
    ...
    def configure_column(self, index, width=None, minwidth=None, anchor=None, stretch=True):
        kwargs = {}
        ...
        if len(data) == len(self._columns):
            ...
            if self.data_type != "text":
                if self.data_type == "int":
                    data = [int(float(x)) if data.index(x)+1 not in self.non_edit_column and x != '-' else x for x in data]
                elif self.data_type == "float":
                    data = [float(x) if data.index(x)+1 not in self.non_edit_column and x != '-' else x for x in data]
        
            self.interior.item(item_ID, values=data)
        else:
            raise ValueError("The multicolumn listbox has only %d columns. %d Supplied"%(self._number_of_columns, len(data)))
    ...
    def insert_row(self, data, index=None):
        ...
        if self.data_type != "text":
            if self.data_type == "int":
                ...
                if self.round == True:
                    data = [round(float(x),2) if data.index(x)+1 not in self.non_edit_column and x != '-' else x for x in data]
                else:
                    data = [float(x) if data.index(x)+1 not in self.non_edit_column and x != '-' else x for x in data]
...
################################################
# The next code is only for TableWidget class
#
#
...
class TableWidget(Frame, object):
    def __init__(self, master, columns, data=None, command=None, editable=False, sort=False, select_mode="multiple", autoscroll=False, vscrollbar=True, hscrollbar=True, heading_anchor = CENTER, cell_anchor=CENTER, style=None, scrollbar_background=None, scrollbar_troughcolor=None, height=None, padding=None, adjust_heading_to_content=False, stripped_rows=None, selection_background=None, selection_foreground=None, cell_background=None, cell_foreground=None, cell_font=None, field_background=None, heading_font= None, heading_background=None, heading_foreground=None, cell_pady=2, column_header=True, row_numbers=True, entry_background="#d6d6d6", entry_foreground=None, entry_validatecommand=None, entry_selectbackground="#1BA1E2", entry_selectborderwidth=None, entry_selectforeground=None, entry_font = "TkDefaultFont", rowlabel_anchor=E, rowlabel_minwidth=0, rowlabel_hoverbackground="#FFFFFF",frame_relief=None, frame_borderwidth=None, frame_background=None, all_column=-1, non_edit=[1], non_edit_row=[], data_type="text", entry_callback=None, log_func=None, round=True, allow_override=True):
        ...
        if hscrollbar:
            ...
            self._hbar=Scrollbar(self,takefocus=0, command=xview_command, orient=tk.HORIZONTAL, **scrollbar_kwargs)
    ...
    def _edit_cell(self, event):
        ...
        if column == "" or column in self._non_editable_columns or self._multicolumn_listbox.interior.index(item_ID) in self._non_editable_row: 
            self.log.add_message("Cannot edit this cell", "RED")
            return
        ...
        self._entry_popup.bind("<FocusOut>", self._on_update_cell)
    ...
    def _on_focus_out(self, event=None):
        print("Focus Out")
        self._on_update_cell()

    def _on_update_cell(self, event=None):
        item_ID, column, column_number = self._selected_cell

        data = self._entry_popup.get()
        
        if data == '-' and self.allow_override == True:
            pass
        ...
        if self.all_column > -1 and column_number == self.all_column:
            for column in row_data[column_number:]:
                row_data[row_data.index(column)] = data
    ...
    def configure_column(self, index, width=None, minwidth=None, anchor=None, stretch=None):
        self._multicolumn_listbox.configure_column(self, index, width=10, minwidth=None, anchor=None, stretch=None)

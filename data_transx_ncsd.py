...
def get_services(filename, op_list, day_filter, date_filter, cur, headways, sim_node_lookup=None, node_lookup=None, model_timings=True):
    ...
    # Dictionary of journey pattern ids to list of journey pattern refs
    j_route_dict = {patt["@id"]:make_list(patt["JourneyPatternSectionRefs"])
            for s in services for patt in s["StandardService"]["JourneyPattern"]}
    
    # Private / internal code dict
    private_code_dict = {j["VehicleJourneyCode"]:j.get("JourneyPatternRef")
            for j in journeys}
    
    # Dictionary of journeys codes to service, pattern, operating days, departure time, valid_days(bool)
    j_dict = {j["VehicleJourneyCode"]:[
            j["ServiceRef"], 
            j.get("JourneyPatternRef", private_code_dict.get(j.get("VehicleJourneyRef"))), 
    ...
    for k, j in j_dict.items():
        ...
        try:
            route_details = []
            for route_detail in j_route_dict[pat_ref]:
                route_details.append(j_pattern_dict[route_detail])
            route_details = [x for sl in route_details for x in sl]
        
        except KeyError as e:
            print("Key Error %s not in " % e, j_pattern_dict.keys())
            
        try:
            line, operator, desc, mode = (s_dict[s_ref][0], s_dict[s_ref][1], 
                                          s_dict[s_ref][4], s_dict[s_ref][5])
        ...
        model_start_time = datetime.strptime(dep_time, time_format) + timedelta(
                seconds=sum(timings[:first_node]))
        ...
        # Classify based on TMfS Rules # # # # # # # # # 
        model_end_time = model_start_time + timedelta(seconds=model_running_time)
        time_format = "%H:%M"
        headway_ranges = [
        ...
        times_in_model = [model_start_time, model_end_time]
        for h_start, h_end in headway_ranges:
            if (all(h_start <= time < h_end for time in times_in_model)):
                running_hour = h_start.time().hour 
                break
            elif (h_start <= times_in_model[0] < h_end - timedelta(minutes=30)):
                running_hour = h_start.time().hour 
                break 
            elif (h_start - timedelta(minutes=30) <= times_in_model[0] < h_start 
                  and h_start + timedelta(minutes=30) < times_in_model[1]):
                running_hour = h_start.time().hour 
                break 
            elif (h_start + timedelta(minutes=30) < times_in_model[1] < h_end):
                running_hour = h_start.time().hour 
                break
        # # # # # # # # # # # # # # # # # # # # # # # # # 

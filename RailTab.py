...
from WidgetTemplates import CreateToolTip, LabelledEntry, TextLog, SelectBox
from WidgetTemplates import ThreadWithReturn
...
class RailTab:
    def __init__(self, note_book, general, name="Rail", default_vars={}):
        ...
        self.days = ["Monday", "Tuesday", "Wednesday", "Thursday",
                     "Friday", "Saturday", "Sunday", "Bank Holiday Running"]
        self.operators = set()
        ...
        self.progress = ttk.Progressbar(
            self.progress_frame, orient=tk.HORIZONTAL, length=500,
            mode="determinate"
        )
    ...
    def create_patching_window(self):
        ...
        def callback_patch_routes():
            try:
                ...
                args = (node_file.get(), link_file.get(),
                        self.to_path("MCA_f"),
        ...
        path_out = os.path.join(self.current_dir,
                                os.path.join(self.gen.inter_dir,
                                             "rail_linked_paths.csv"))
        err_out = os.path.join(self.current_dir,
                               os.path.join(self.gen.inter_dir,
                                            "rail_unlinked_paths.csv"))
        ...
        node = LabelledEntry(
            file_frame, "Node File", node_file, w=50,
        ...
        link = LabelledEntry(
            file_frame, "Link File", link_file, w=50,
        ...
        progress = ttk.Progressbar(
            log_frame, orient=tk.HORIZONTAL, length=200, mode="determinate")
        progress.pack(side="top", padx=5, pady=5)

    def read_file(self, key):
        if key == "read":
            ri.import_station_names(self.to_path(
                "MSN"), self.to_path("stn"), self.log)
            ...
            threading.Thread(target=ri.import_cif_callback,
                             args=(f_args)).start()
            ...
            try:
                ...
                self.log.add_message(("No operator file present\nTry "
                                      "importing some data first"),
                                     color="RED")
            self.update_input(ops)
        elif key == "fil":
            custom_headways = (self.gen.head_defs.get(),
                               self.gen.head_name.get())
            ri.CIF_post_filter("rail", self.to_path("MCA_a"),
                               self.to_path("MCA_f"),
                               self.to_path(
                                   "lin"), self.used_operators.get_contents(),
                               self.gen.to_path(
                                   "ops"), self.gen.to_path("mode"),
                               self.log)
    ...
    def create_filter_widgets(self, frame):
        ...
        label = ttk.Label(
            frame,
            text=("Enter desired file to save filtered services "
    ...
    def create_operator_widgets(self, frame):
        ...
        ttk.Label(
            frame,
            text=("-> Read in the operator list then move the "
                  "desired operators to the 'Keep' pane\nAll "
                  "'Remove' operators will be filtered out")
        ).grid(row=0, column=1, sticky="w", columnspan=3)
    ...
    def create_print_lin_widgets(self, frame):
        ...
        ttk.Button(
            frame, text="4. Print LIN File", style="DS.TButton",
            command=lambda: self.read_file("lin")
        ).pack(
            side="top", padx=5, pady=5, ipadx=5, ipady=5,
    ...
    def create_file_widgets(self, frame):
        ...
        a = LabelledEntry(
            frame, "MCA File", self.files["MCA"], w=50,
        ...
        a = LabelledEntry(
            frame, "Rolling Stock", self.files["stock"], w=50,
        ...
        a = LabelledEntry(
            frame, "TIPLOC Lookup", self.files["t_name"], w=50,
    ...
    def add_frames(self):
        ...
        self.file_select_frame = add_frame(
            self.master, "Select Input and Output files",
            [2, 1], c_span=2, s="we"
        )
    ...
    def set_defaults(self):
        f = self.files
        f["MCA"].set(r"..\Data\Rail\Rail Data\Test1.mca")
        f["MSN"].set(r"..\Data\Rail\Rail Data\Test1.msn")
        f["MCA_a"].set(
            os.path.relpath(
                os.path.join(self.gen.inter_dir, "mca_out_head.csv")
            )
        )
        f["MCA_f"].set(
            os.path.relpath(
        ...
        f["MCA_p"].set(
            os.path.relpath(
                os.path.join(
                    self.gen.inter_dir, "mca_out_head_patched.csv")
            )
        )
        f["stn"].set(
            os.path.relpath(
                os.path.join(self.gen.inter_dir, "stat_name.csv")
            )
        )

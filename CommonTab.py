...
#        self.create_headway_widgets(self.headway_frame)
    ...
    def create_GUI_widgets(self, frame, load_func, save_func):
        ...
        l = ttk.Button(frame, text="Load Default Variables", command=lambda:load_func())
        l.grid(row=1,column=1)
        s = ttk.Button(frame, text="Save Default Variables", command=lambda:save_func())
        s.grid(row=1,column=2)
        
    def create_headway_widgets(self, frame):
        description = ttk.Label(frame, text=("-> Specify Headway Periods Using "
                                             "a Comma Separated List\n e.g. "
                                             "7-10, 10-16, 16-19 as AM, Mid, PM"))
        description.grid(row=1, column=0,sticky=tk.W,columnspan=3)
    ...
    def create_date_widgets(self, frame):
        ...
        l = ttk.Label(frame, text="-> Select the date range of interest")
        l.grid(row=0,column=0,sticky="w")
        CreateToolTip(l, text=("This is a single date to avoid clashes in "
    ...
    def create_day_widgets(self, frame):
        ...
        for i in range(0, len(self.days)):
            day_boxes.append(ttk.Radiobutton(frame, text=self.days[i], 
                                             variable=self.selected_day, 
                                             value=i, command=self.update_day_list)
                             .grid(row=rw, column=clm, sticky=tk.W))
            rw += 1
            if rw > len(self.days) / 2:
                clm += 1
                rw = 1 
#        ttk.Checkbutton(frame, text="Bank Holidays", variable=self.bank_hol,
#                        onvalue=1,offvalue=0).grid(row=rw,column=clm,stick=tk.W)
    ...
    def create_file_widgets(self, frame):
        ...
        def set_file_path(variable):
            ...
            if path is not None:
                try:
                    self.files[variable].set(os.path.relpath(path, self.current_dir))
                except ValueError:
                    return
        ...
        CreateToolTip(a, text="Contains a one to one mapping of 'ATCOCode' to 'Node'")
        ttk.Entry(frame, width=50, textvariable=self.files["b_nod"]).grid(column=0,row=2)
        ttk.Button(frame, text="Browse", 
                   command=lambda : set_file_path("b_nod")).grid(column=1,row=2)
    ...
    def update_date(self, index, value, op):
        ...
        try:
            date(day=int(new_date[0]), month=int(new_date[1]), year=int(new_date[2]))
